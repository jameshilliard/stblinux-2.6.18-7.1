...
{
	unsigned long flags;
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_MIPS_BCM7325)
...
#if defined(HUMAX_PLATFORM_BASE) && defined(CONFIG_MIPS_BCM7325)
	/*Disable scb-transaction combining feature for both EHCI and OHCI.*/
	/*BRCM have confirmed that this patch doesn't affect it's performance*/
	BDEV_UNSET(BCHP_USB_CTRL_EBRIDGE,
		 BCHP_USB_CTRL_EBRIDGE_EBR_SEQ_EN_MASK);
	BDEV_RD(BCHP_USB_CTRL_EBRIDGE);

	BDEV_UNSET(BCHP_USB_CTRL_OBRIDGE,
		BCHP_USB_CTRL_OBRIDGE_OBR_SEQ_EN_MASK);
	BDEV_RD(BCHP_USB_CTRL_OBRIDGE);	

	/*Avoid OUT underflows under heavy AV processing by setting EHCI bridge FIFO threshold to 256 bytes*/
	/* and EHCI core threshold for Rx to 256 bytes and Tx to 512 bytes*/
	/*BRCM said that this patch affect R/W performance slightly. (around 1%)*/
	reg_temp  = BDEV_RD(BCHP_USB_CTRL_EBRIDGE);
	reg_temp = ((reg_temp & ~BCHP_USB_CTRL_EBRIDGE_EBR_SCB_SIZE_MASK) | 
				( 8 << BCHP_USB_CTRL_EBRIDGE_EBR_SCB_SIZE_SHIFT));
	BDEV_WR(BCHP_USB_CTRL_EBRIDGE, reg_temp);

	BDEV_WR(BCHP_USB_EHCI_INSNREG01, 0x00800040);
	BDEV_WR(BCHP_USB_EHCI_INSNREG03, 1);

	BDEV_WR(BCHP_USB_EHCI1_INSNREG01, 0x00800040);
	BDEV_WR(BCHP_USB_EHCI1_INSNREG03, 1);	

	/*BRCM Recommendation: Reset each OHCI and power down their ports to prevent detecting failure for some bus powered devices*/
	BDEV_WR(BCHP_USB_OHCI_HcCommandStatus, 1);
	BDEV_WR(BCHP_USB_OHCI_HcControl, 0);
	
	BDEV_WR(BCHP_USB_OHCI1_HcCommandStatus, 1);
	BDEV_WR(BCHP_USB_OHCI1_HcControl, 0);		
#endif
