...
#define NAND_BLOCK_STATUS_GOOD	0
#define NAND_BLOCK_STATUS_BAD	1
	
#define BLOCK_SIZE_128K			(128*1024)

#if CONFIG_MTD_BRCMNAND_SIZE == 256
#define NANDFLASH_SIZE			(256*8*BLOCK_SIZE_128K)
#elif CONFIG_MTD_BRCMNAND_SIZE == 128
#define NANDFLASH_SIZE			(128*8*BLOCK_SIZE_128K)
#else
#error
#endif
#define NAND_CACHE_SIZE		512

/* Nand Parition Definition */
// Block Management Table
#define BMTforGang_SIZE 		BLOCK_SIZE_128K							// Block Management Table For Gang
#define BMTforFusing_SIZE		(2*BLOCK_SIZE_128K)						// Block Management Table For Fusing
#define BMT_Reserved_SIZE		(5*BLOCK_SIZE_128K)						//RFU
#define BMTPART_SIZE			(BMTforGang_SIZE + BMTforFusing_SIZE + BMT_Reserved_SIZE)	// Block Management Table Size

// Linux Kernel Size
#define KERNEL_SIZE 				(2*8*BLOCK_SIZE_128K)					// 2Mbytes
#define ROOTFS_SIZE				(16*8*BLOCK_SIZE_128K)					// 16Mbytes
#define OTA_AREA_SIZE			(18*8*BLOCK_SIZE_128K)					// 18Mbytes
#define RFU_SIZE					(1*8*BLOCK_SIZE_128K)					// 1MByte


#define RESERVED_REPLACEMENT	(20*BLOCK_SIZE_128K)					// Reserved AREA (For Data Replace)
#define RESERVED_AREA			(20*BLOCK_SIZE_128K)					// Reserved AREA (This reserved block need to make BMT for gang)
#define RESERVED_BLOCK			(RESERVED_REPLACEMENT + RESERVED_AREA)	// 5MBytes

// Store Area (yaffs2)
#define FREESPACE_SIZE			(NANDFLASH_SIZE - (BMTPART_SIZE  + KERNEL_SIZE + ROOTFS_SIZE + OTA_AREA_SIZE + RFU_SIZE + RESERVED_BLOCK ))
#define MIDDLE_OFFSET			(BMTPART_SIZE + KERNEL_SIZE + ROOTFS_SIZE + OTA_AREA_SIZE + RFU_SIZE + RESERVED_BLOCK )

#define NAND_BLOCKNUM			(NANDFLASH_SIZE/BLOCK_SIZE_128K)		// 256*8 = 2048

#define RESERVED_AREA_OFFSET	(MIDDLE_OFFSET - RESERVED_BLOCK)
...
/**************************************************************************
	BMT Information
***************************************************************************
	Block 0 : Bad block map table for gang.
	Block 1 : Bad block map table for fusing.
	Block 2 : Bad block map table for fusing (backup).
**************************************************************************/
#define BMT_FOR_GANG_OFFSET				0x00000000;	//block 0
#define BMT_FOR_FUSING_OFFSET			0x00020000;	//block 1
#define BMT_FOR_FUSING_BACKUP_OFFSET	0x00040000;	//block 2
#define BMT_SIZE						(2*NAND_BLOCKNUM) //  (2*2048)
#define BMT_CRC_SIZE					2
#define PAGE_SIZE						2048

static int blockmap_1[NAND_BLOCKNUM]; // For Gang
static int blockmap_2[NAND_BLOCKNUM]; // For Download
static int blockmap_enable = 0;

int nand_mark_block_bad (struct mtd_info *mtd, loff_t ofs);
unsigned int nand_replace_find(void);
static void nand_makeBMTforFusing(unsigned char *buf_ptr, int size);
#endif
...
#if defined(HUMAX_PLATFORM_BASE)
	{	/* 31 */
		.chipId = MICRON_MT29F1G08ABADA,
		.mafId = FLASHTYPE_MICRON,
		.chipIdStr = "MICRON_MT29F1G08ABADA",
		.options = NAND_USE_FLASH_BBT,
		.idOptions = BRCMNAND_ID_EXT_BYTES,
		.timing1 = 0, .timing2 = 0,
...
#if defined(HUMAX_PLATFORM_BASE)
static unsigned short GetCRC16(unsigned char *BufferPtr, unsigned long NumberOfByte);
static int nand_get_block_status(int blk_num, int blk_size, int pg_size, uint32_t *value );
...
#if defined(HUMAX_PLATFORM_BASE)
	if (nandCtrlReg < BCHP_NAND_REVISION ||(nandCtrlReg & 0x3) != 0) {
#else
...
#if 0 //defined(HUMAX_PLATFORM_BASE)
	if(ldw < RESERVED_AREA_OFFSET)
	{
		if(blockmap_enable)
		{
			pagenum		= ldw%(BLOCK_SIZE_128K);
			blocknum	= ldw/(BLOCK_SIZE_128K);

			//block address remapping (Download)
			blocknum	= blockmap_2[blocknum];

			//block address remapping (Gang)
			blocknum	= blockmap_1[blocknum];

			ldw =blocknum*(BLOCK_SIZE_128K) + pagenum;
		}
	}
#endif
 ...
 */
#if defined(HUMAX_PLATFORM_BASE)
int brcmnand_ctrl_verify_ecc(struct brcmnand_chip* chip, int state, uint32_t notUsed)
#else
...
#if defined(HUMAX_PLATFORM_BASE)
	if(from >= 0x100000 && (from+len) <= 0x2600000) /*Allow reading kernel and rootfs areas, too*/
//	if(from >= 0x1300000 && (from+len) <= 0x2600000)
	{
		if(blockmap_enable)
		{
			pagenum		= from%(BLOCK_SIZE_128K);
			blocknum	= from/(BLOCK_SIZE_128K);
			...
			from =blocknum*(BLOCK_SIZE_128K) + pagenum;
		}
		else
		{
			printk("[brcmnand_read] bmt not enabled.\n");
			return -1;
		}			
	}
	else if(from >= 0 && from < 0x100000)
	{
		printk("[brcmnand_read] access not allowed. (addr=0x%lx len=%ld)\n", from, len);
		return -1;
	}
#endif
			  ...
			  size_t *retlen, const uint8_t *buf)
{
	struct brcmnand_chip *chip = mtd->priv;
#if defined(HUMAX_PLATFORM_BASE)
	loff_t replaced_block_addr;
	int	i;
	unsigned char	buff[BMT_SIZE];
	uint32_t pagenum, blocknum;
...
printk("Attempt to write beyond end of device\n");	
	...
	if(to >= 0x100000 && (to+len) <= 0x2600000)
#else
	if(to >= 0x1300000 && (to+len) <= 0x2600000)
#endif
	{
		if(blockmap_enable)
		{
			InRange = 17;
			pagenum		= to%(BLOCK_SIZE_128K);
			blocknum	= to/(BLOCK_SIZE_128K);
			...
			to =blocknum*(BLOCK_SIZE_128K) + pagenum;
		}
		else
		{
			printk("[brcmnand_write] bmt not enabled.\n");
			return -1;
		}		
	}
	else if(to >= 0x0 && to < 0x1300000)
	{
		printk("[brcmnand_write] access not allowed. (addr=0x%lx len=%ld)\n", to, len);
		return -1;
	}
#endif
...
#if defined(HUMAX_PLATFORM_BASE)
	if(InRange == 17) //(to >= 0x1300000 && (to+len) < 0x2600000)
	{
		if(ret != 0)
		{
			/*Write error occured.*/
			ret = nand_mark_block_bad (mtd, to);

			if(ret != 0)
			{
		  		DEBUG(MTD_DEBUG_LEVEL0, "%s: ", __FUNCTION__,
				      "Failed to mark block bad.\n");		 	
			}
			else
			{
				replaced_block_addr = (loff_t) nand_replace_find();/*find reserved area, return : block_address*/
				if(replaced_block_addr == -1) //ERROR
				{
			  		DEBUG(MTD_DEBUG_LEVEL0, "%s: ", __FUNCTION__,
					      "No reserved areas available.\n");
				}
				else
				{
					ret = brcmnand_do_write_ops(mtd, replaced_block_addr, &chip->ops);
					if (ret == 0)
					{
						blockmap_2[to/BLOCK_SIZE_128K] = replaced_block_addr/BLOCK_SIZE_128K;


						for(i=0; i< NAND_BLOCKNUM; i++)
						{
							buff[(2*i)]	= (unsigned char)(blockmap_2[i] >> 8);		//256 : The number of page * 4
							buff[(2*i+1)]	= (unsigned char)(blockmap_2[i] & 0xFF);
						}
						nand_makeBMTforFusing(buff, BMT_SIZE);	
					}
...
#if defined(HUMAX_PLATFORM_BASE)
#if defined(CONFIG_FACTORY)
	if( instr->addr >= 0x100000 && ( instr->addr+instr->len) <= 0x2600000)
#else
	if( instr->addr >= 0x1300000 && ( instr->addr+instr->len) <= 0x2600000)
#endif	
	{
		if(blockmap_enable && instr->len == 0x20000)
		{
			pagenum	= instr->addr%(BLOCK_SIZE_128K);
			blocknum	= instr->addr/(BLOCK_SIZE_128K);
			...
			instr->addr =blocknum*(BLOCK_SIZE_128K) + pagenum;
		}
		else
		{
			printk("[brcmnand_erase] bmt not enabled or length error.(len=0x%lx)\n", instr->len);
			return -1;
		}
	}
	else if(instr->addr >= 0x0 && instr->addr < 0x1300000)
	{
		printk("[brcmnand_erase] access not allowed. (addr=0x%lx len=%ld)\n", instr->addr, instr->len);
		return -1;
	}	
#endif
	/* Grab the lock and see if the device is available */
...
int nand_mark_block_bad (struct mtd_info *mtd, loff_t ofs)
{
	...
	ret = brcmnand_block_checkbad(mtd, ofs, 0, 1);
	if (ret) {
		/* If it was bad already, return success and do nothing */
		if (ret > 0)
			return 0;
	...
	blk_addrs = blk_addrs & ~(blk_size-1);

	/* first read the spare area*/
	rval = do_nand_cmd (NAND_CMD_PAGE_READ, blk_addrs);

	/* copy all the spare aread regiseters */

	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_0) = REG(BCHP_NAND_SPARE_AREA_READ_OFS_0);
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_4) = REG(BCHP_NAND_SPARE_AREA_READ_OFS_4);
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_8) = REG(BCHP_NAND_SPARE_AREA_READ_OFS_8);
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_C) = REG(BCHP_NAND_SPARE_AREA_READ_OFS_C);

	if ( pg_size >= 2*1024 )
	{
		temp = REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_0);
		SET_REG_FIELD(temp, BCHP_NAND, SPARE_AREA_WRITE_OFS_0, BYTE_OFS_0, 0x00);
		REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_0) = temp;
	}
	else
	{
		temp = REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_4);
		SET_REG_FIELD(temp, BCHP_NAND, SPARE_AREA_WRITE_OFS_4, BYTE_OFS_5, 0x00);
		REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_4) = temp;
	}

	do_nand_cmd (NAND_CMD_PROGRAM_SPARE_AREA, blk_addrs);

	//Refresh NAND Spare Area Write Buffer.
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_0) = 0xFFFFFFFF;
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_4) = 0xFFFFFFFF;
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_8) = 0xFFFFFFFF;
	REG(BCHP_NAND_SPARE_AREA_WRITE_OFS_C) = 0xFFFFFFFF;
#endif
}

int nand_read_block(uint32_t blockbase,unsigned char *buf_ptr,int block_size)
{
	int			i, j, page_num;
	int			virpgsize = 512;
	uint32_t	nand_intfc_status;
	uint32_t	*reg_src;

	reg_src		= (uint32_t *)buf_ptr;
	page_num	= block_size/virpgsize;

	if((block_size%virpgsize)!=0)
		page_num++;

	if((blockbase%virpgsize)!=0)
		page_num++;

	for(i=0; i < page_num; i++)
	{
		brcmnand_ctrl_write(BCHP_NAND_CMD_ADDRESS, blockbase + i*virpgsize);
		brcmnand_ctrl_write(BCHP_NAND_CMD_START, BCHP_NAND_CMD_START_OPCODE_PAGE_READ << BCHP_NAND_CMD_START_OPCODE_SHIFT);

		for(;;)
		{
			nand_intfc_status = brcmnand_ctrl_read(BCHP_NAND_INTFC_STATUS);
			if ((nand_intfc_status & BCHP_NAND_INTFC_STATUS_CTLR_READY_MASK) != 0)
			{
				break;
			}
		}
		...
		for(j=0; j<128; j++)
		{
			*reg_src = brcmnand_ctrl_read(BCHP_NAND_FLASH_CACHEi_ARRAY_BASE+j*4);
			reg_src++;
		}
	}

	return (nand_intfc_status & 0x1);
}

int nand_write_block(uint32_t blockbase,unsigned char *buf_ptr,int block_size)
{
	int			i, j, page_num;
	int			virpgsize = 512;
	uint32_t	nand_intfc_status;
	...
	for(i=0; i < page_num; i++)
	{
		...
		for(j=0; j<128; j++)
		{
			brcmnand_ctrl_write(BCHP_NAND_FLASH_CACHEi_ARRAY_BASE+j*4, *reg_src);
			reg_src++;
		}
	
		brcmnand_ctrl_write(BCHP_NAND_CMD_ADDRESS, blockbase + i*virpgsize);
		brcmnand_ctrl_write(BCHP_NAND_CMD_START, BCHP_NAND_CMD_START_OPCODE_PROGRAM_PAGE << BCHP_NAND_CMD_START_OPCODE_SHIFT);
...
int nand_erase_block(uint32_t blockbase)
{
	uint32_t	nand_intfc_status;
	int rval;

	rval = nand_get_block_status (blockbase/BLOCK_SIZE_128K, BLOCK_SIZE_128K, 2048, 0);
	if ( rval == 0 )
	{
		brcmnand_ctrl_write(BCHP_NAND_CMD_ADDRESS, blockbase);
		brcmnand_ctrl_write(BCHP_NAND_CMD_START, BCHP_NAND_CMD_START_OPCODE_BLOCK_ERASE << BCHP_NAND_CMD_START_OPCODE_SHIFT);
...
static int nand_get_block_status(int blk_num, int blk_size, int pg_size, uint32_t *value )
{
	uint32_t	temp,nand_intfc_status;
	uint8_t		rval;
	int32_t		blk_addrs;

	blk_addrs = blk_num*blk_size;

	//Input Address at NAND_CMD_ADDRESS
	brcmnand_ctrl_write(BCHP_NAND_CMD_ADDRESS, blk_addrs);
	//Input NAND_Command at NAND_CMD_START register
	brcmnand_ctrl_write(BCHP_NAND_CMD_START,BCHP_NAND_CMD_START_OPCODE_SPARE_AREA_READ<<BCHP_NAND_CMD_START_OPCODE_SHIFT);
	...
	// 2KB Page => Byte of (1 or 0)(in spare area) is bad block indicator, 512B page => Byte of 5(in spare area)
	if (pg_size >= 2048)
	{
		temp = brcmnand_ctrl_read(BCHP_NAND_SPARE_AREA_READ_OFS_0);
		rval = (uint8_t)((temp&BCHP_NAND_SPARE_AREA_READ_OFS_0_BYTE_OFS_0_MASK) >> BCHP_NAND_SPARE_AREA_READ_OFS_0_BYTE_OFS_0_SHIFT);
	}
	else
	{
		temp = brcmnand_ctrl_read(BCHP_NAND_SPARE_AREA_READ_OFS_4);
		rval = (uint8_t)((temp&BCHP_NAND_SPARE_AREA_READ_OFS_4_BYTE_OFS_5_MASK) >> BCHP_NAND_SPARE_AREA_READ_OFS_4_BYTE_OFS_5_SHIFT);
	}
...
unsigned int nand_replace_find(void) // Find block for backup, return : valid block
{
	uint32_t block_addr, nand_intfc_status, temp, i;
	int	blocksize	= BLOCK_SIZE_128K;
	
	block_addr = (MIDDLE_OFFSET-RESERVED_BLOCK);
	for(i=block_addr; i<MIDDLE_OFFSET-RESERVED_AREA; i+=blocksize)
	{
		brcmnand_ctrl_write(BCHP_NAND_CMD_ADDRESS, i);
		brcmnand_ctrl_write(BCHP_NAND_CMD_START,BCHP_NAND_CMD_START_OPCODE_SPARE_AREA_READ<<BCHP_NAND_CMD_START_OPCODE_SHIFT);
		for(;;)
		{
			nand_intfc_status = brcmnand_ctrl_read(BCHP_NAND_INTFC_STATUS);
...
#if 0
static int32_t nand_handle_write_error(nandflashpart_t *part, uint32_t bad_block_address, uint8_t* data)
{
	nandflashdev_t *softc = part->fp_dev;

	uint32_t block_size=softc->fd_probe.flash_block_size;
	uint32_t page_size=softc->fd_probe.flash_pg_size;
	uint32_t rval;
	uint32_t temp_block_address;
	uint8_t* copy_buffer;

	nand_mark_block_bad (bad_block_address, block_size, page_size );
	temp_block_address = nand_replace_find();/*find reserved area, return : block_address*/
	if(temp_block_address == -1) //ERROR
	{
		return -1;
	}
	...
	rval = nand_write_block(temp_block_address, copy_buffer, copy_buffer, 0, block_size, page_size, NAND_ACCESS_MIDDLE_BLOCK, 0);
	...
	nand_blk_map[bad_block_address/block_size] = temp_block_address/block_size;
	return 0;
}
#endif

unsigned char	h1_block_tag[BMT_SIZE];
static uint32_t nand_findLastBMT(uint32_t blockbase)
{
	uint32_t		offset = 0;
	uint32_t		last_table_offset = 0;
	unsigned short	BMT_stored_crc = 0;
	unsigned short	BMT_calculated_crc = 0;

	for(offset = 0; offset<BLOCK_SIZE_128K; offset+=(BMT_SIZE))
	{
		nand_read_block(blockbase+offset, h1_block_tag, BMT_SIZE);
		BMT_stored_crc		= (h1_block_tag[0] << 8) | h1_block_tag[1];
		BMT_calculated_crc	= GetCRC16(h1_block_tag + BMT_CRC_SIZE, BMT_SIZE - BMT_CRC_SIZE);
		if(BMT_stored_crc == BMT_calculated_crc)
		{
	...
	return last_table_offset;
}

unsigned char	h2_block_tag[BMT_SIZE];
unsigned char	h2_blank_block_tag[BMT_SIZE];
static uint32_t nand_findBlankBMT(uint32_t blockbase)
{
	uint32_t		offset;
	uint32_t		blank_block_offset = 0;


	memset(h2_blank_block_tag, 0xFF, BMT_SIZE);

	offset = nand_findLastBMT(blockbase);
#if 1
	for(offset = offset+BMT_SIZE; offset<BLOCK_SIZE_128K; offset+=(BMT_SIZE))
	{
		nand_read_block(blockbase, h2_block_tag, BMT_SIZE);

		if(memcmp(h2_blank_block_tag, h2_block_tag, BMT_SIZE) == 0)
		{
			break;
		}
	}
#endif
	blank_block_offset = offset;


	return blank_block_offset;
}


unsigned char	h0_buff[BMT_SIZE];
static void nand_makeBMTforFusing(unsigned char *buf_ptr, int size) // Make Block Management Table for Fusing
{
	int				i;
	int				status;
	...
	uint32_t		blockbase;
	unsigned short	BMT_Calculated_crc = 0;
	unsigned short	BMT_Readed_crc = 0;
	uint8_t* read_buffer = (uint8_t*) kmalloc(BLOCK_SIZE_128K, GFP_ATOMIC);;


	
	BMT_Calculated_crc = GetCRC16(buf_ptr+BMT_CRC_SIZE , size-BMT_CRC_SIZE);
	buf_ptr[0]	= (BMT_Calculated_crc >> 8) & 0xFF;
	buf_ptr[1]	= BMT_Calculated_crc & 0xFF;

	blockbase	= BMT_FOR_FUSING_OFFSET;

	// Need to Check block 1 & 2 : Whether these blocks is Bad block or not,
	for(i = 0; i < 2;i++)
	{
		blockbase	= blockbase + (i*BLOCK_SIZE_128K);
		status		= nand_get_block_status((blockbase/BLOCK_SIZE_128K), BLOCK_SIZE_128K, 2048, 0);
		if(status == NAND_BLOCK_STATUS_GOOD)
		{
			//Read block tag : if tag == "11", write block inform at next page
			
			table_offset = nand_findBlankBMT(blockbase);
#if 1
#if 0
			if(table_offset < BLOCK_SIZE_128K)	// Find Blank block
			{
				nand_write_block(blockbase+table_offset, buf_ptr, size);
			}
			else							// not found
#endif
			{
				table_offset = 0x1000;
				nand_erase_block(blockbase);
				nand_write_block(blockbase+table_offset, buf_ptr, size);
			}

			memset(h0_buff, 0xFF, size);

			if(nand_read_block(blockbase, h0_buff, size) != NAND_BLOCK_STATUS_GOOD)
			{
				continue;
			}

			BMT_Calculated_crc	= GetCRC16(h0_buff+BMT_CRC_SIZE , size-BMT_CRC_SIZE);
			BMT_Readed_crc		= (h0_buff[0] << 8) | h0_buff[1];

			if(BMT_Calculated_crc != BMT_Readed_crc)
			{
				continue;
			}
#endif			
		...
		{
			printk("#######################################");
			printk("##### Block Management Table Block[%d] is BAD!!!####", i+1);
			printk("#######################################");
		}
	}
	kfree(read_buffer);
...
void brcmnand_call_blockmap(struct mtd_info *mtd)
{
	bool			bBMTforFursingOK = TRUE;
	uint8_t			*buf_ptr = 0;
	int				i = 0, tag = 0;
	...
	uint16_t	Readed_CRC = 0;

	struct brcmnand_chip *this = mtd->priv;

	size_t readed_size = 0;

	buf_ptr = vmalloc (BMT_SIZE);
	if (!buf_ptr) {
		printk (KERN_ERR "%s: Out of memory size=%dx\n", __FUNCTION__, BMT_SIZE);
		return -ENOMEM;
	}

	// Block Map_1 for Image Gang
	block_offset = BMT_FOR_GANG_OFFSET;
	if(nand_get_block_status(block_offset/blocksize, blocksize, pagesize, 0) == NAND_BLOCK_STATUS_GOOD)
	{
		nand_read_block(block_offset, buf_ptr, BMT_SIZE);

		Calculated_CRC	= GetCRC16(buf_ptr+BMT_CRC_SIZE, BMT_SIZE-BMT_CRC_SIZE);
		Readed_CRC		= (buf_ptr[0] << 8) | buf_ptr[1];
	...
	{
			printk("[brcmnand_call_blockmap] CRC not matched - Calculated : 0x%04X VS. Readed : 0x%04X\n", Calculated_CRC, Readed_CRC);
			tag = 0;
		}
	}
	else
	{
		printk("[brcmnand_call_blockmap] BMT For gang Broken!!\n");
		tag = 0;
	}

	blocknum	= (MIDDLE_OFFSET-RESERVED_AREA) / blocksize;
	memset(blockmap_1, 0xFF, sizeof(blockmap_1));
	if(tag == 1)
	{
		for(i=0;i<blocknum;i++)
			blockmap_1[i] = (buf_ptr[i*2]<<8) + buf_ptr[i*2+1];
	}
	else
	{
		printk("[brcmnand_call_blockmap] none of Mapping data\n");
		for(i=0;i<blocknum;i++)
			blockmap_1[i] = i;
	}
	...
	for(i= 0; i<blocknum; i++)
	{
		if((blockmap_1[i]%128) == 0)
			printk(KERN_INFO "## Block[%d] is maped at Block[%d]\n", i, blockmap_1[i]);
		if(blockmap_1[i] >= (blocknum-1))
			break;
	}
#endif

	// Block Map_2 for Image Download
	tag = 0;
	block_offset = BMT_FOR_FUSING_OFFSET;
	if(nand_get_block_status(block_offset/blocksize, blocksize, pagesize, 0) == NAND_BLOCK_STATUS_BAD)
	{
		block_offset = BMT_FOR_FUSING_BACKUP_OFFSET;
		if(nand_get_block_status(block_offset/blocksize, blocksize, pagesize, 0) == NAND_BLOCK_STATUS_BAD)
		{
			printk("[brcmnand_call_blockmap] BMT For Fusing Broken!!\n");
			bBMTforFursingOK = FALSE;
		}
	}

	if(bBMTforFursingOK)
	{
		for(i=0; i<blocksize; i+=BMT_SIZE)
		{
			nand_read_block(block_offset+i, buf_ptr, BMT_SIZE);
	...
	blocknum	= (MIDDLE_OFFSET-RESERVED_BLOCK) / blocksize;
	memset(blockmap_2, 0xFF, sizeof(blockmap_2));
	if(tag == 1)
	{
		nand_read_block(block_offset+last_block_offset, buf_ptr, BMT_SIZE);

		for(i=0;i<blocknum;i++)
			blockmap_2[i] = (buf_ptr[i*2]<<8) + buf_ptr[i*2+1];
	}
	else
	{
		printk("[brcmnand_call_blockmap] none of Mapping data\n");
	...
	for(i= 0; i<blocknum; i++)
	{
		if((blockmap_2[i]%128) == 0)
			printk(KERN_INFO "## Block[%d] is maped at Block[%d]\n", i, blockmap_2[i]);
		if(blockmap_2[i] >= (blocknum-1))
			break;
	}
#endif
...
int brcmnand_scan(struct mtd_info *mtd , int maxchips )
{
	...
	//Chip Select
	cs				= 0;
	chip->CS[cs]	= NAND_CHIP_SELECT;
	chip->numchips	= 1;

	chip->eccsize	= 512;  // Fixed for Broadcom controller

	if (brcmnand_probe(mtd, chip->CS[cs]))
		return -ENXIO;
#else
