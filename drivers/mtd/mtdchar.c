...
static int otp_select_filemode(struct mtd_file_info *mfi, int mode)
{
	...
	case MTD_OTP_USER:
		...
		break;
#if defined(HUMAX_PLATFORM_BASE)
	case MTD_OTP_USER_SET_PASSWORD:
	case MTD_OTP_USER_PROGRAM_PASSWORD:
		if (!mtd->write_password_prot_reg)
		{
			ret = -EOPNOTSUPP;		
			printk("MTD_OTP_USER_PROGRAM_PASSWORD-Error\n");			
		}
		else
		{
			mfi->mode = mode;
			printk("MTD_OTP_USER_PROGRAM_PASSWORD-OK");
			
		}
		break;
	case MTD_OTP_USER_GET_EXT_INFO:
		if (!mtd->read_ext_info)
		{
			ret = -EOPNOTSUPP;			
			printk("MTD_OTP_USER_GET_EXT_INFO-Error\n");
		}
		else
		{
			mfi->mode = mode;
			printk("MTD_OTP_USER_GET_EXT_INFO-OK");
		}
		break;
#endif
	default:
